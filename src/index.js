...
searchForm.addEventListener("submit", event => {
  ...
  fetch(openWeatherApp)
    ...
    .catch(() => {
      warning.textContent =
        "I'm afraid we don't know the weather of this city.";
    });
...
let toggleTempF = (elem, cc, tempInfo) => {
  let cel = `<i class='fas fa-thermometer-three-quarters'></i>${Math.round(
    tempInfo
  )}°C`;
  let faren = `<i class='fas fa-thermometer-three-quarters'></i>${Math.round(
    tempInfo * (9 / 5) + 32
  )}°F`;
  if (checkTempF) {
    cc.innerHTML = cel;
...
const domContainer = data => {
  ...
  iconImg.src = `http://openweathermap.org/img/wn/${
    info.getWeather().icon
  }@2x.png`;
  ...
  extraInfo[1].innerHTML = `<i class='fas fa-temperature-high c-red'></i>
            <span class='x-padd'>Max temp</span>${Math.round(
              info.getTemp().temp_max
            )}°C`;

  extraInfo[2].innerHTML = `<i class='fas fa-temperature-low c-blue'></i>
            <span class='x-padd'>Min temp</span>${Math.round(
              info.getTemp().temp_min
            )}°C`;
  ...
  b.addEventListener("click", () => {
    toggleTemp(b, a, info.getTemp().temp);
  });
  d.addEventListener("click", () => {
    toggleTempF(d, c, info.getTemp().temp);
  });
};
